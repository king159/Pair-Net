...
class DualTransformer(BaseModule):
    ...
    def __init__(self, encoder=None, decoder1=None, decoder2=None, init_cfg=None):
        ...
        self.attn_factor = (
            torch.diagflat(torch.full((100,), 1e-6)).requires_grad_(True).cuda()
        )
    ...
    def forward(self, x, mask, query1_embed, query2_embed, pos_embed):
        ...
        # first decoder for object query
        out_dec1 = self.decoder1(
            query=target1,
            key=memory,
            value=memory,
            key_pos=pos_embed,
        ...
        # second decoder for relation query
        out_dec2 = self.decoder2(
            query=target2,
            key=memory,
            value=memory,
            key_pos=pos_embed,
