...
class DirectionAwareMessagePassing(nn.Module):
    ...
    def get_attention(self, obj_feat, union_feat, rel_pair_idx):
        ...
        atten_coeff = self.w(
            ...
            * self.wo(obj_feat[rel_pair_idx[:, 1]])
            * self.wu(union_feat)
        )
        ...
        if atten_tensor.shape[0] == 1:
            atten_tensor_sum = torch.ones(atten_tensor_sum.size()).to(atten_tensor_sum)
    ...
    def forward(self, obj_feats, union_feats, det_result):
        ...
        obj_rep = torch.cat(
            (obj_feats, obj_embed, pos_embed), -1
        )  # N x (1024 + 200 + 128)
        ...
        for obj_feat, union_feat, rel_pair_idx in zip(
            split_obj_rep, split_union_rep, rel_pair_idxes
        ):
            atten_tensor = self.get_attention(
                obj_feat, union_feat, rel_pair_idx
            )  # N x N x 1
            atten_tensor_t = torch.transpose(atten_tensor, 1, 0)
            atten_tensor = torch.cat(
                (atten_tensor, atten_tensor_t), dim=-1
            )  # N x N x 2
