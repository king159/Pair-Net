...
class IMPContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes):
        ...
        self.sub_vert_w_fc = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid()
        )
        self.obj_vert_w_fc = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid()
        )
        self.out_edge_w_fc = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid()
        )
        self.in_edge_w_fc = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid()
        )
    ...
    def forward(self, x, union_features, det_result, logger=None):
        ...
        for pair_idx, num_obj in zip(rel_pair_idxes, num_objs):
            ...
            rel_idx = (
                torch.arange(num_rel).to(obj_rep.device).long().view(-1) + rel_offset
            )
        ...
        hx_obj = torch.zeros(obj_count, self.hidden_dim, requires_grad=False).to(
            obj_rep
        )
        hx_rel = torch.zeros(rel_count, self.hidden_dim, requires_grad=False).to(
            obj_rep
        )
        ...
        for i in range(self.num_iter):
            ...
            weighted_sub = (
                self.sub_vert_w_fc(torch.cat((sub_vert, edge_factor[i]), 1)) * sub_vert
            )
            weighted_obj = (
                self.obj_vert_w_fc(torch.cat((obj_vert, edge_factor[i]), 1)) * obj_vert
            )
            ...
            pre_out = (
                self.out_edge_w_fc(torch.cat((sub_vert, edge_factor[i]), 1))
                * edge_factor[i]
            )
            pre_in = (
                self.in_edge_w_fc(torch.cat((obj_vert, edge_factor[i]), 1))
                * edge_factor[i]
            )
