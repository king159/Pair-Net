...
def get_size_maps(size, boxes_int, form="rect"):
    ...
    # boxes_int = bbox.long()
    boxes_w, boxes_h = (
        boxes_int[:, 2] - boxes_int[:, 0] + 1,
        boxes_int[:, 3] - boxes_int[:, 1] + 1,
    )
    ...
    for rel_pair_idx, gt_bbox, det_bbox, saliency_map in zip(
        ...
        object_saliency = (
            torch.cat(
                [
                    torch.mean(
                        saliency_map[0, box[1] : box[3] + 1, box[0] : box[2] + 1]
                    )[None]
            ...
            .float()
            .to(det_bbox_int.device)
        )
        object_area = get_size_maps(saliency_map.shape[1:], det_bbox_int, area_form)
        object_importance = (
            object_saliency * comb_factor + (1.0 - comb_factor) * object_area
        )
        pair_importance = (
            object_importance[rel_pair_idx[:, 0]]
            + object_importance[rel_pair_idx[:, 1]]
        )
