...
class RelationSampler(object):
    ...
    def prepare_test_pairs(self, det_result):
        ...
        for p in det_result.bboxes:
            n = len(p)
            cand_matrix = torch.ones((n, n), device=device) - torch.eye(
                n, device=device
            )
            ...
            if self.test_overlap:
                cand_matrix = (
                    cand_matrix.byte() & bbox_overlaps(p[:, :4], p[:, :4]).gt(0).byte()
                )
    ...
    def gtbox_relsample(self, det_result, gt_result):
        ...
        gt_bboxes, gt_labels, gt_relmaps, gt_rels, gt_keyrels = (
            gt_result.bboxes,
            gt_result.labels,
            gt_result.relmaps,
            gt_result.rels,
        ...
        for img_id, (
            ...
            if tgt_keyrel is not None:
                img_keyrel_labels = (
                    torch.zeros(tgt_pair_idxs.shape[0]).long().to(tgt_pair_idxs.device)
                )
            ...
            if tgt_pair_idxs.shape[0] > num_pos_per_img:
                perm = torch.randperm(tgt_pair_idxs.shape[0], device=device)[
                    :num_pos_per_img
                ]
    ...
    def motif_rel_fg_bg_sampling(
        ...
        for i in range(num_tgt_rels):
            ...
            tgt_key_rel_lab = (
                int(img_keyrel_labels[i]) if img_keyrel_labels is not None else None
            )
            ...
            if tgt_key_rel_lab is not None:
                fg_key_labels = torch.tensor(
                    [tgt_key_rel_lab] * prp_tail_idxs.shape[0],
                    dtype=torch.int64,
                    device=device,
                ).view(-1, 1)
            ...
            if fg_rel_i.shape[0] > self.num_sample_per_gt_rel:
                ious_score = (
                    (
                        ious[tgt_head_idx, prp_head_idxs]
        ...
        bg_rel_triplets = torch.cat((bg_rel_inds, bg_rel_labs.view(-1, 1)), dim=-1).to(
            torch.int64
        )
        if self.key_sample:
            bg_key_labels = (
                torch.tensor(bg_rel_inds.shape[0], dtype=torch.int64, device=device)
                .fill_(-1)
                .view(-1, 1)
            )
        ...
        num_neg_per_img = min(
            self.num_rel_per_image - fg_rel_triplets.shape[0], bg_rel_triplets.shape[0]
        )
        if bg_rel_triplets.shape[0] > 0:
            ...
            bg_rel_triplets = torch.zeros(
                (0, 4 if self.key_sample else 3), dtype=torch.int64, device=device
            )
