    ...
    def forward(self, tree, features, num_obj):
        ...
        if self.dropout > 0.0:
            dropout_mask = get_dropout_mask(
                self.dropout, (1, self.out_dim), features.device
            )
        else:
            dropout_mask = None

        # tree lstm input
        h_order = torch.tensor(
            [0] * num_obj, device=features.device, dtype=torch.int64
        )  # used to resume order
...
class BiTreeLSTM_Foreward(nn.Module):
    ...
    def node_forward(self, feat_inp, left_c, right_c, left_h, right_h, dropout_mask):
        ...
        ioffu = (
            self.ioffux(feat_inp)
            + self.ioffuh_left(left_h)
            + self.ioffuh_right(right_h)
        )
    ...
    def forward(self, tree, features, treelstm_io):
        ...
        if tree.left_child is None:
            left_c = (
                torch.tensor([0.0] * self.h_dim, device=features.device)
                .float()
                .view(1, -1)
            )
            left_h = (
        ...
        if self.is_pass_embed:
            next_feature = torch.cat(
                (
                    features[tree.index].view(1, -1),
                    left_embed.view(1, -1),
                    right_embed.view(1, -1),
                ),
                1,
            )
        else:
        ...
        c, h = self.node_forward(
            next_feature, left_c, right_c, left_h, right_h, treelstm_io.dropout_mask
        )
        tree.state_c = c
        tree.state_h = h
        ...
        # Only being used in decoder network
        if self.is_pass_embed:
            pass_embed_postprocess(
                h,
                self.embed_out_layer,
                self.embed_layer,
                tree,
                treelstm_io,
                self.training,
...
class BiTreeLSTM_Backward(nn.Module):
    ...
    def forward(self, tree, features, treelstm_io):
        ...
        c, h = self.node_backward(
            next_features, root_c, root_h, treelstm_io.dropout_mask
        )
...
def pass_embed_postprocess(
    h, embed_out_layer, embed_layer, tree, treelstm_io, is_training
):
