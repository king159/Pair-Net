...
class DecoderTreeLSTM(nn.Module):
    def __init__(
        ...
        embed_vecs = obj_edge_vectors(
            ["start"] + self.classes, wv_dir=self.cfg.glove_dir, wv_dim=embed_dim
        )
        ...
        if direction == "backward":
            ...
            self.decoderLSTM = BiTreeLSTM_Backward(
                self.input_size,
                self.hidden_size,
                is_pass_embed=True,
            ...
            self.decoderLSTM = BiTreeLSTM_Foreward(
                self.input_size,
                self.hidden_size,
                is_pass_embed=True,
                embed_layer=self.obj_embed,
    ...
    def forward(self, tree, features, num_obj):
        ...
        if self.dropout > 0.0:
            dropout_mask = get_dropout_mask(
                self.dropout, (1, self.hidden_size), features.device
            )
        else:
            dropout_mask = None
...
class VCTreeLSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes):
        ...
        self.decoder_rnn = DecoderTreeLSTM(
            self.cfg,
            self.obj_classes,
    ...
    def obj_ctx(
        self, num_objs, obj_feats, obj_labels=None, vc_forest=None, ctx_average=False
    ):
        ...
        obj_labels = (
            obj_labels.split(num_objs, dim=0) if obj_labels is not None else None
        )
    ...
    def forward(self, x, det_result, all_average=False, ctx_average=False):
        ...
        bi_inp = torch.cat(
            (
                self.obj_reduce(x.detach()),
                self.emb_reduce(obj_embed.detach()),
                box_inp,
                pair_inp,
            ),
            -1,
        )
        bi_preds, vc_scores = self.vctree_score_net(
            num_objs, bi_inp, obj_dists
        )  # list of N x N
        forest = generate_forest(vc_scores, det_result)
        vc_forest = arbForest_to_biForest(forest)

        # object level contextual feature
        obj_ctxs, obj_preds, obj_dists = self.obj_ctx(
            num_objs, obj_pre_rep, obj_labels, vc_forest, ctx_average=ctx_average
        )
        ...
        if (
            ...
            obj_rel_rep = torch.cat(
                (self.untreated_edg_feat.view(1, -1).expand(batch_size, -1), obj_ctxs),
                dim=-1,
            )
        else:
    ...
    def vctree_score_net(self, num_objs, roi_feat, roi_dist):
        ...
        for sub, obj, dist in zip(sub_feats, obj_feats, roi_dists):
            ...
            num_obj = sub.shape[0]
            num_dim = sub.shape[-1]
            sub = sub.view(1, num_obj, num_dim).expand(
                num_obj, num_obj, num_dim
            )  # N, N, 512
            obj = obj.view(num_obj, 1, num_dim).expand(
                num_obj, num_obj, num_dim
            )  # N, N, 512
            sub_dist = (
                dist.view(1, num_obj, -1).expand(num_obj, num_obj, -1).unsqueeze(2)
            )  # N, N, 1, 151
            obj_dist = (
                dist.view(num_obj, 1, -1).expand(num_obj, num_obj, -1).unsqueeze(3)
            )  # N, N, 151, 1
            joint_dist = (sub_dist * obj_dist).view(
                num_obj, num_obj, -1
            )  # N, N, (151, 151)
