...
def generate_forest(pair_scores, det_result):
    ...
    for pair_score, label, bbox in zip(
        ...
        for idx in list(range(num_obj)):
            if idx == root_idx:
                continue
            new_node = ArbitraryTree(
                idx, float(node_scores[idx]), int(obj_label[idx]), bbox[idx]
            )
...
class ArbitraryTree(object):
    ...
    def search_best_insert(
        ...
        for i in range(self.get_child_num()):
            best_score, best_depend_node, best_insert_node = self.children[
                i
            ].search_best_insert(
                score_map, best_score, insert_node, best_depend_node, best_insert_node
            )
