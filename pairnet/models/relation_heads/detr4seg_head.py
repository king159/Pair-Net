...
class detr4segHead(AnchorFreeHead):
    ...
    def forward(self, feats, img_metas):
        ...
        last_features = feats[-1]  ####get feature outputs of intermediate layers
        ...
        seg_masks = seg_masks.view(
            batch_size, self.num_query, seg_masks.shape[-2], seg_masks.shape[-1]
        )
    ...
    def loss_single(
        ...
        dice_loss = self.dice_loss(
            mask_preds, mask_targets, num_matches
        )  # ,s_mask_weights,avg_factor=num_total_pos)
    ...
    def get_bboxes(self, cls_scores, bbox_preds, img_metas, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            triplets = self._get_bboxes_single(
                cls_score, bbox_pred, mask_pred, img_shape, scale_factor, rescale
            )
    ...
    def _get_bboxes_single(
        self, cls_score, bbox_pred, mask_pred, img_shape, scale_factor, rescale=False
    ):
        ...
        det_masks = F.interpolate(
            det_masks.unsqueeze(1), size=mask_size, mode="bilinear"
        ).squeeze(1)
