...
class GPSHead(RelationHead):
    def __init__(self, **kwargs):
        ...
        self.out_rel = nn.Linear(
            self.head_config.roi_dim, self.num_predicates, bias=True
        )
    ...
    def forward(
        ...
        pair_reps = []
        pair_preds = []
        for pair_idx, obj_rep, obj_pred in zip(
            det_result.rel_pair_idxes, split_roi_context_feats, obj_preds
        ):
            pair_preds.append(
                torch.stack((obj_pred[pair_idx[:, 0]], obj_pred[pair_idx[:, 1]]), dim=1)
            )
            pair_reps.append(
                torch.cat((obj_rep[pair_idx[:, 0]], obj_rep[pair_idx[:, 1]]), dim=-1)
            )
        ...
        if self.use_bias:
            log_freq = F.log_softmax(
                self.freq_bias.index_with_labels(pair_preds.long() - 1)
            )  # USE 0-index when getting frequency vec!
            if log_freq.isnan().any():  # TODO:why?
                log_freq = None

        rel_scores = self.relation_infer(
            pair_reps,
            union_feats,
            self.w_proj1,
            self.w_proj2,
        ...
        if not is_testing:
            ...
            det_result.target_rel_labels = (
                torch.cat(det_result.target_rel_labels, dim=-1)
                if det_result.target_rel_labels is not None
                else None
            )
        ...
        if self.with_relation_ranker:
            det_result = self.relation_ranking_forward(
                pair_reps, det_result, gt_result, num_rels, is_testing
            )
        return det_result
