...
class PSGFormerHead(AnchorFreeHead):
    ...
    def forward(self, feats, img_metas, train_mode=False):
        ...
        subject_scores = (
            torch.matmul(rel_q_normalized, sub_q_normalized.transpose(1, 2)) / self.temp
        )
        object_scores = (
            torch.matmul(rel_q_normalized, obj_q_normalized.transpose(1, 2)) / self.temp
        )
        ...
        all_cls_scores = dict(
            cls=outputs_class, sub=sub_outputs_class, obj=obj_outputs_class
        )
    ...
    def loss_single(
        ...
        # id loss
        filtered_subject_scores = torch.cat(filtered_subject_scores, 0).reshape(
            len(filtered_subject_scores[0]), -1
        )
        ...
        gt_object_id = F.one_hot(
            gt_object_id, num_classes=filtered_object_scores.shape[-1]
        )
        loss_subject_match = self.sub_id_loss(filtered_subject_scores, gt_subject_id)
        loss_object_match = self.obj_id_loss(filtered_object_scores, gt_object_id)
    ...
    def _get_bboxes_single(
        ...
        cat_bbox_pred = torch.cat([bbox_pred, bbox_pred], dim=0)
        cat_cls_score = (
            torch.cat([all_cls_score, all_cls_score], dim=0).max(axis=-1).indices
        )
        ...
        if self.use_mask:
            ...
            return cat_bbox_pred, cat_cls_score, rel_pairs, r_scores, r_labels, r_dists
