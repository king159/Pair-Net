...
class MyPSGFormerHead(AnchorFreeHead):
    _version = 2
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if train_cfg:
            assert "id_assigner" in train_cfg, (
                "id_assigner should be provided " "when train_cfg is set."
            )
            assert "bbox_assigner" in train_cfg, (
                "bbox_assigner should be provided " "when train_cfg is set."
            )
            ...
            assert loss_bbox["loss_weight"] == bbox_assigner["reg_cost"]["weight"], (
    ...
    def _init_layers(self):
        ...
        self.obj_query_embed = nn.Embedding(self.num_obj_query, self.embed_dims)
        self.rel_query_embed = nn.Embedding(self.num_rel_query, self.embed_dims)
        ...
        self.rel_cls_embed = nn.Sequential(
            Linear(256, self.embed_dims),
            nn.ReLU(inplace=True),
            Linear(self.embed_dims, self.rel_cls_out_channels),
        )

        self.bbox_attention = MHAttentionMap(
            self.embed_dims, self.embed_dims, self.n_heads, dropout=0.0
        )
        self.mask_head = MaskHeadSmallConv(
            self.embed_dims + self.n_heads, [1024, 512, 256], self.embed_dims
        )
    ...
    def forward(self, feats, img_metas, train_mode=False):
        ...
        masks = (
            F.interpolate(masks.unsqueeze(1), size=last_features.shape[-2:])
            .to(torch.bool)
            .squeeze(1)
        )
        # position encoding
        ...
        outs_obj_dec, outs_rel_dec, memory = self.transformer(
            last_features,
            masks,
        ...
        cls_preds = self.class_embed(outs_obj_dec)
        outputs_coord = self.box_embed(outs_obj_dec).sigmoid()
        bbox_mask = self.bbox_attention(outs_obj_dec[-1], memory, mask=masks)
        seg_masks = self.mask_head(
            last_features, bbox_mask, [feats[2], feats[1], feats[0]]
        )
        seg_masks = seg_masks.view(
            batch_size, self.num_obj_query, seg_masks.shape[-2], seg_masks.shape[-1]
        )

        ### interaction
        updated_sub_embed = self.sub_query_update(outs_obj_dec)
        updated_obj_embed = self.obj_query_update(outs_obj_dec)
        sub_q_normalized = F.normalize(updated_sub_embed[-1], p=2, dim=-1, eps=1e-12)
        obj_q_normalized = F.normalize(updated_obj_embed[-1], p=2, dim=-1, eps=1e-12)
        self.rel_query_update is nn.Identity
        updated_rel_embed = self.rel_query_update(outs_rel_dec)
        rel_q_normalized = F.normalize(updated_rel_embed[-1], p=2, dim=-1, eps=1e-12)
        ...
        num_layer = outputs_coord.shape[0]

        sub_outputs_class = torch.empty_like(cls_preds)
        sub_outputs_coord = torch.empty_like(outputs_coord)
        obj_outputs_class = torch.empty_like(cls_preds)
        obj_outputs_coord = torch.empty_like(outputs_coord)
        sub_seg = torch.empty_like(seg_masks)
        obj_seg = torch.empty_like(seg_masks)
        triplet_sub_ids = []
        triplet_obj_ids = []
        for i in range(len(sub_ids)):
            triplet_sub_id = sub_ids[i]
            triplet_obj_id = obj_ids[i]
            sub_outputs_class[:, i] = cls_preds[:, i, triplet_sub_id, :]
            sub_outputs_coord[:, i] = outputs_coord[:, i, triplet_sub_id, :]
            obj_outputs_class[:, i] = cls_preds[:, i, triplet_obj_id, :]
            obj_outputs_coord[:, i] = outputs_coord[:, i, triplet_obj_id, :]
            sub_seg[i] = seg_masks[i, triplet_sub_id, :, :]
            obj_seg[i] = seg_masks[i, triplet_obj_id, :, :]
            triplet_sub_ids.append(triplet_sub_id)
            triplet_obj_ids.append(triplet_obj_id)

        rel_preds = self.rel_cls_embed(outs_rel_dec)
        rel_preds = rel_preds.unsqueeze(0).repeat(num_layer, 1, 1, 1)

        all_cls_scores_list = dict(
            # object detection
            sub=sub_outputs_class,  # (6,b,100,134)
            obj=obj_outputs_class,
            cls=cls_preds,  # (6,b,100,134)
            # relation detection
            rel=rel_preds,  # (6,b,100,57)
            sub_ids=triplet_sub_ids,  # list of (100)
            obj_ids=triplet_obj_ids,
            subject_scores=subject_scores,  # (b,100,100)
            object_scores=object_scores,  # same
        )
        all_bbox_preds_list = dict(
            bbox=outputs_coord,  # (6,b,100,4)
            sub=sub_outputs_coord,  # (6,b,100,4)
            obj=obj_outputs_coord,
            mask=seg_masks,
            sub_seg=sub_seg,  # (b,100,h/4,w/4)
            obj_seg=obj_seg,
        )

        return all_cls_scores_list, all_bbox_preds_list
    ...
    def loss_single(
        ...
        for (
            ...
            loss_subject_match = self.sub_id_loss(filtered_subject_score, gt_subject_id)
            loss_object_match = self.obj_id_loss(filtered_object_score, gt_object_id)
        ...
        cls_avg_factor = num_total_od_pos * 1.0 + num_total_od_neg * self.bg_cls_weight
        if self.sync_cls_avg_factor:
            cls_avg_factor = reduce_mean(od_cls_scores.new_tensor([cls_avg_factor]))
        cls_avg_factor = max(cls_avg_factor, 1)

        loss_cls = self.loss_cls(
            od_cls_scores, labels, label_weights, avg_factor=cls_avg_factor
        )

        # Compute the average number of gt boxes across all gpus, for
        # normalization purposes
        num_total_od_pos = loss_cls.new_tensor([num_total_od_pos])
        num_total_od_pos = torch.clamp(reduce_mean(num_total_od_pos), min=1).item()
        ...
        loss_iou = self.loss_iou(
            bboxes, bboxes_gt, bbox_weights, avg_factor=num_total_od_pos
        )

        # regression L1 loss
        loss_bbox = self.loss_bbox(
            od_bbox_preds, bbox_targets, bbox_weights, avg_factor=num_total_od_pos
        )
        ...
        if self.sync_cls_avg_factor:
            cls_avg_factor = reduce_mean(r_cls_scores.new_tensor([cls_avg_factor]))
        cls_avg_factor = max(cls_avg_factor, 1)
    ...
    def _get_target_single(
        ...
        od_assign_result = self.bbox_assigner.assign(
            bbox_pred, cls_score, gt_bboxes, gt_labels, img_meta, gt_bboxes_ignore
        )
        sampling_result = self.sampler.sample(od_assign_result, bbox_pred, gt_bboxes)
        ...
        # match id targets
        gt_subject_ids = gt_sub_bboxes.new_full((num_rels,), -1, dtype=torch.long)
        gt_subject_ids[pos_inds] = gt_sub_ids[s_sampling_result.pos_assigned_gt_inds]

        gt_object_ids = gt_obj_bboxes.new_full((num_rels,), -1, dtype=torch.long)
        ...
        r_labels = gt_obj_bboxes.new_full((num_rels,), 0, dtype=torch.long)  ### 1-based
    ...
    def _get_bboxes_single(
        ...
        if self.use_mask:
            ...
            all_masks = F.interpolate(all_masks.unsqueeze(1), size=mask_size).squeeze(1)
