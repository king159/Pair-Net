...
def compute_gqa_coco_overlap(output_path: Path):
    ...
    coco_thing_cats = load_json(Path("data/coco/detectron/thing_categories.json"))
    coco_stuff_cats = load_json(Path("data/coco/detectron/stuff_categories.json"))
...
def compute_coco_gqa_overlap(
    ...
    for gqa_d, coco_d in tqdm(dataset):
        ...
        gqa_annos = [
            (anno["category_id"], anno["bbox"]) for anno in gqa_d["annotations"]
        ]
...
def psg_to_kaihua(
    ...
    for d in tqdm(dataset):
        ...
        for s in d["segments_info"]:
            ...
            boxes_1024.append(
                xyxy_to_xcycwh(resize_bbox(old_height, old_width, s["bbox"], 1024))
            )
            boxes_512.append(
                xyxy_to_xcycwh(resize_bbox(old_height, old_width, s["bbox"], 512))
            )

            labels.append(
                s["category_id"] + 1 if s["isthing"] else s["category_id"] + 81
            )
    ...
    coco_val_ids = set([p.stem.lstrip("0") for p in coco_val_img_dir.glob("*.jpg")])
