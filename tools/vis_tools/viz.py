...
def viz_annotations(
    ...
    if return_graph:
        ...
        for idx, obj in enumerate(annos):
            g.node(
                f"obj_{idx}", metadata.thing_classes[obj["category_id"]], color="orange"
            )
...
def viz_annotations_alt(
    ...
    if n_rels is not None:
        ...
        n_rels = len([r for r in data["relations"] if r[2] not in rel_ids_to_filter])
...
def viz_annotations_psg(
    ...
    if type == "masks":
        # Load panoptic segmentation
        seg_map = read_image(data_dir / data["pan_seg_file_name"], format="RGB")
        # Convert to segment ids
        seg_map = rgb2id(seg_map)
